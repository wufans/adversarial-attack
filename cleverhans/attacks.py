...
class FastGradientMethod(Attack):
    ...
    def parse_params(self, eps=0.3, ord=np.inf, y=None, y_target=None,
        ...
        return True
class FastGradientMethod_adv(Attack):
    ...
    def __init__(self, model, back='tf', sess=None):
        ...
        cleverhans.model.Model abstraction provided by CleverHans.
        """
        super(FastGradientMethod_adv, self).__init__(model, back, sess)
        self.feedable_kwargs = {'eps': np.float32,
                                'y': np.float32,
                                'y_target': np.float32,
                                'clip_min': np.float32,
    ...
    def generate(self, x, **kwargs):
        ...
        from .attacks_tf import fgm1

        labels, nb_classes = self.get_or_guess_labels(x, kwargs)

        return fgm1(x, self.model.get_probs(x), y=labels, eps=self.eps,
                   ord=self.ord, clip_min=self.clip_min,
                   clip_max=self.clip_max,
                   targeted=(self.y_target is not None))
